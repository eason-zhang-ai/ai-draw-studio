...
interface GraphvizChatMessageDisplayProps {
    messages: UIMessage[];
    error?: Error | null;
...
export function GraphvizChatMessageDisplay({
    messages,
    error,
    setInput,
    setFiles,
}: GraphvizChatMessageDisplayProps) {
    const { updateDefinition } = useGraphviz();
    ...
    useEffect(() => {
        messages.forEach((message) => {
            message.parts?.forEach((part: any) => {
                ...
                if (
                    part.type === "tool-display_graphviz" &&
