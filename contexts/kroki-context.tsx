...
// Default definitions for each diagram type following official kroki.io formats
...
(singleton) ---> (model)`,
    ...
    mermaid: `graph TD
  A[Anyone] --> B{Can contribute?}
  B -->|Yes| C[Contribute]
  B -->|No| D[Learn]
  C --> E[Make a pull request]
  D --> F[Fork the repository]
  F --> C`,
...
interface KrokiContextValue {
    definition: string;
    history: KrokiHistoryEntry[];
    updateDefinition: (definition: string, summary?: string, diagramType?: string) => void;
    setDefinition: (definition: string) => void;
    clearDefinition: () => void;
    initialDefinition: string;
    setDefinitionForType: (type: string) => void;
}

const KrokiContext = createContext<KrokiContextValue | undefined>(
    undefined
);
...
export function KrokiProvider({ children }: { children: React.ReactNode }) {
    ...
    const updateDefinition = useCallback(
        (nextDefinition: string, summary?: string, diagramType?: string) => {
    ...
    const clearDefinition = useCallback(() => {
        setDefinitionState(DEFAULT_DEFINITIONS.plantuml);
        setHistory([
            createHistoryEntry(DEFAULT_DEFINITIONS.plantuml, "Diagram reset"),
        ]);
    }, []);
    ...
    const value = useMemo(
        () => ({
            ...
            setDefinition,
            clearDefinition,
            initialDefinition: DEFAULT_DEFINITIONS.plantuml,
            setDefinitionForType,
        }),
        [definition, history, updateDefinition, setDefinition, clearDefinition, setDefinitionForType]
    );

    return (
        <KrokiContext.Provider value={value}>
            {children}
        </KrokiContext.Provider>
    );
}

export function useKroki() {
    const context = useContext(KrokiContext);
    if (!context) {
        throw new Error("useKroki must be used within a KrokiProvider");
    }
    return context;
}